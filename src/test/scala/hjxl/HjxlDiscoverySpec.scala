...
class HjxlDiscoverySpec extends AnyFreeSpec with Matchers with ChiselSim {
  ...
  "focused discovery reports its compile-time trace stage" in {
    ...
    simulate(new HjxlRouteDiscoveryHarness(traceRoute = TraceStage.AqFuzzyErosion)) { dut =>
      expectRoute(
        dut,
        RouteConfig(xyb = true, quant = true, tokenSelect = TokenTraceSelect.AqContrast),
        TraceStage.AqFuzzyErosion
      )
    }
    simulate(new HjxlRouteDiscoveryHarness(traceRoute = TraceStage.AqStrategyMask)) { dut =>
      expectRoute(
        dut,
        RouteConfig(xyb = true, quant = true, tokenSelect = TokenTraceSelect.AqContrast),
        TraceStage.AqStrategyMask
      )
    }
    simulate(new HjxlRouteDiscoveryHarness(traceRoute = TraceStage.AqNonlinearMask)) { dut =>
      expectRoute(
        dut,
        RouteConfig(xyb = true, quant = true, tokenSelect = TokenTraceSelect.AqContrast),
        TraceStage.AqNonlinearMask
      )
    }
    simulate(new HjxlRouteDiscoveryHarness(traceRoute = TraceStage.AqHfModulation)) { dut =>
      expectRoute(
        dut,
        RouteConfig(xyb = true, quant = true, tokenSelect = TokenTraceSelect.AqContrast),
        TraceStage.AqHfModulation
      )
    }
    simulate(new HjxlRouteDiscoveryHarness(traceRoute = TraceStage.AqColorModulation)) { dut =>
      expectRoute(
        dut,
        RouteConfig(xyb = true, quant = true, tokenSelect = TokenTraceSelect.AqContrast),
    ...
    simulate(new HjxlRouteDiscoveryHarness(traceRoute = TraceStage.AqFinalMap)) { dut =>
      expectRoute(
        dut,
        RouteConfig(xyb = true, quant = true, tokenSelect = TokenTraceSelect.AqContrast),
        TraceStage.AqFinalMap
      )
    }
  }
