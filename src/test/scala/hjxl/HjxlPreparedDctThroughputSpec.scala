...
class HjxlPreparedDctThroughputSpec extends AnyFreeSpec with Matchers with ChiselSim {
  ...
  "the direct prepared-DCT stream has measured cycle and token-expansion budgets" in {
    simulate(new HjxlPreparedDctAxiStreamCore(config)) { dut =>
      ...
      zeroOne mustBe Measurement("zero-8x8", 8, 8, 1, 1, 201, 201, 0, 2, 12, 3, 1, 5, 3, 140, 128, 342)
      impulseOne mustBe
        Measurement("three-ac-8x8", 8, 8, 1, 1, 201, 201, 0, 2, 15, 3, 1, 5, 6, 145, 130, 347)
      zeroTwo mustBe
        Measurement("zero-16x8", 16, 8, 2, 1, 402, 403, 1, 2, 22, 6, 2, 8, 6, 207, 185, 611)
