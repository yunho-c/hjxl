...
def _register_writes(manifest: dict) -> list[tuple[str, int, int, int]]:
...
def _input_data_bytes(manifest: dict) -> int:
    return (_input_data_bits(manifest) + 7) // 8
...
def header_text(manifest: dict, *, symbol_prefix: str, include_guard: str | None) -> str:
    ...
    lines = [
        ...
        f"#define {prefix}_INPUT_DATA_BITS {input_data_bits}u",
        f"#define {prefix}_STREAM_WORD_BYTES {input_data_bytes}u",
        f"#define {prefix}_STREAM_BYTE_COUNT {word_count * input_data_bytes}u",
